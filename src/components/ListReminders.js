...
const ListReminders = ({userEmail, arrayReminder, setArrayReminder}) => {
    const deleteReminder = async (idDeleteReminder) => {
        ...
        const newArrayReminders = arrayReminder.filter((objReminder => objReminder.id !== idDeleteReminder))
                ...
                {arrayReminder.map((objReminder) => {
                    return (
                        ...
                        <Row>
                            ...
                            <Col><Button variant="danger" onClick={()=> deleteReminder(objReminder.id)} >Eliminar recordatorio</Button></Col>
                        </Row>
                        <hr />
                        </>
                    )
